...
export class B2BService {
  ...
  async createOrder(userId: string, dto: CreateB2BOrderDto) {
    ...
    const grouped=new Map<string,number>();for(const item of dto.items){if(!Number.isSafeInteger(item.quantity)||item.quantity<1)throw new BadRequestException('Некорректное количество товара');const quantity=(grouped.get(item.variantId)||0)+item.quantity;if(quantity>10000)throw new BadRequestException('Слишком большое количество товара');grouped.set(item.variantId,quantity);}dto={...dto,items:[...grouped].map(([variantId,quantity])=>({variantId,quantity}))};
    const ids = [...new Set(dto.items.map(item => item.variantId))]; const variants = await this.prisma.productVariant.findMany({ where: { id: { in: ids }, isActive: true, product:{isActive:true,productType:'PHYSICAL'} }, include: { product: true } }); if (variants.length !== ids.length) throw new NotFoundException('Один из товаров не найден или выключен');
    ...
    const order = await this.prisma.$transaction(async tx => { for (const line of [...lines].sort((a,b)=>a.variant.id.localeCompare(b.variant.id))) { const rows=await tx.$queryRaw<{id:string}[]>`UPDATE "ProductVariant" SET reserved = reserved + ${line.item.quantity} WHERE id = ${line.variant.id} AND "isActive" = true AND stock - reserved >= ${line.item.quantity} RETURNING id`;if(rows.length!==1)throw new BadRequestException(`Недостаточно товара: ${line.variant.product.nameRu}`); } const orderNumber = `SB-B2B-${new Date().getFullYear()}-${randomUUID().slice(0,12).toUpperCase()}`; return tx.order.create({ data: { orderNumber, userId, customerId: member.customerId, organizationId: member.organizationId, source: OrderSource.B2B, sourceChannel: 'B2B', status: OrderStatus.NEW, reservationState:'ACTIVE', buyerName: [member.user.firstName, member.user.lastName].filter(Boolean).join(' '), buyerEmail: member.user.email, buyerPhone: member.user.phone, totalAmount: total, discountAmount: total - final, finalAmount: final, shippingAddress: (dto.shippingAddress || { address: member.organization.legalAddress || '' }) as Prisma.InputJsonValue, comments: dto.comments, items: { create: lines.map(line => ({ variantId: line.variant.id, externalSku: line.variant.sku, productName: line.variant.product.nameRu, variantName: line.variant.name, price: line.price, quantity: line.item.quantity, total: line.total })) }, history: { create: { toStatus: OrderStatus.NEW, changedBy: userId, comment: 'B2B-заказ создан в кабинете организации' } } }, include: { items: true } }); });
    await this.oneC.enqueueOrder(order.id, userId);
    return protectB2BFinance(order,member.canSeeFinance);
  }

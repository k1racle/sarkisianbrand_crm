...
export class PlatformChatService {
  ...
  async postMessage(channelId: string, dto: CreatePlatformMessageDto, userId: string) {
    ...
    const attachments = await this.resolveEntities(dto.entities || []);
  ...
  async postUpload(channelId: string, files: any[], body: string | undefined, entitiesSource: string | undefined, replyToId: string | undefined, userId: string) {
    ...
    const entityAttachments = await this.resolveEntities(entities);
  ...
  async searchEntities(type: string, search = '') {
    const query = search.trim();
    if (type === 'TASK') {
      const rows = await this.prisma.task.findMany({ where: { status: { not: TaskStatus.CANCELLED }, ...(query ? { OR: [{ title: { contains: query, mode: 'insensitive' } }, { description: { contains: query, mode: 'insensitive' } }] } : {}) }, include: { assignedTo: { select: { firstName: true, lastName: true, email: true } } }, orderBy: { updatedAt: 'desc' }, take: 20 });
    ...
    if (type === 'CUSTOMER') {
      const rows = await this.prisma.customer.findMany({ where: query ? { OR: [{ firstName: { contains: query, mode: 'insensitive' } }, { lastName: { contains: query, mode: 'insensitive' } }, { email: { contains: query, mode: 'insensitive' } }, { phone: { contains: query } }] } : {}, orderBy: { updatedAt: 'desc' }, take: 20 });
    ...
    if (type === 'STAGE') {
      const rows = await this.prisma.crmPipelineStage.findMany({ where: query ? { name: { contains: query, mode: 'insensitive' } } : {}, include: { pipeline: true, _count: { select: { leads: true } } }, orderBy: { sortOrder: 'asc' }, take: 20 });
    ...
    if (type === 'PRODUCT') {
      const rows = await this.prisma.product.findMany({ where: { isActive: true, ...(query ? { OR: [{ nameRu: { contains: query, mode: 'insensitive' } }, { sku: { contains: query, mode: 'insensitive' } }] } : {}) }, include: { images: { take: 1 } }, orderBy: { updatedAt: 'desc' }, take: 20 });
  ...
  private async resolveEntities(entities: PlatformEntityAttachmentDto[]) {
    const result: any[] = [];
    for (const entity of entities.slice(0, 8)) {
      ...
      const matches = await this.searchEntities(entity.type);

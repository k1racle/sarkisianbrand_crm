...
export class ShippingService {
  constructor(private readonly prisma: PrismaService, private readonly provider: ShippingProvider, private readonly oneC: OneCSyncService) {}
  ...
  async createShipment(orderNumber: string, dto: ShippingAddressDto) {
    ...
    await this.prisma.order.update({ where: { id: order.id }, data: { shippingAddress: dto as unknown as Prisma.InputJsonValue, shippingProvider: shipment.provider, trackingNumber: shipment.trackingNumber, shippingCost: quote.amount, status: OrderStatus.SHIPPED, isSynced1C: false } });

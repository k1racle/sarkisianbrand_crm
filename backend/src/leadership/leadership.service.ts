...
export class LeadershipService {
  ...
  async overview() {
    ...
    const [current, previous, customers, newCustomers, lowStock, openLeads, activeTasks, helpdeskOpen, helpdeskOverdue, recentOrders, channelGroups] = await Promise.all([
      this.prisma.order.aggregate({ where: { createdAt: { gte: currentFrom }, paymentStatus: 'SUCCEEDED', status: { notIn: [...closedStatuses] } }, _count: true, _sum: { finalAmount: true }, _avg: { finalAmount: true } }),
      this.prisma.order.aggregate({ where: { createdAt: { gte: previousFrom, lt: currentFrom }, paymentStatus: 'SUCCEEDED', status: { notIn: [...closedStatuses] } }, _count: true, _sum: { finalAmount: true } }),
      ...
      this.prisma.order.groupBy({ by: ['source'], where: { createdAt: { gte: currentFrom }, paymentStatus: 'SUCCEEDED', status: { notIn: [...closedStatuses] } }, _count: true, _sum: { finalAmount: true } }),
    ]);
    ...
    return {
      ...
      sales: { revenue, orders, averageOrder: orders ? revenue / orders : 0, previousRevenue, growth, basis: 'CONFIRMED_PAYMENTS' },

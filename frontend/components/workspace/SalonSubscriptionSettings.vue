...
<template>
 <main class="salon-subscription-page crm-standard"><header class="crm-page-header"><h1>Подписка для салонов</h1><button class="crm-button crm-button--refresh" type="button" :disabled="loading||busy" @click="load"><RefreshCw :size="18"/>Обновить</button></header>
  <p v-if="error" role="alert">{{error}}</p><p v-if="loading" role="status">Загружаем настройки…</p>
  <form v-else-if="baseline" class="salon-settings-panel crm-surface" @submit.prevent="save"><h2>Тариф кабинета и онлайн-записи</h2><p>Сейчас функции доступны всем салонам бесплатно. Цены и режим ниже — подготовка будущей подписки: платежи, списания и блокировка доступа не активированы.</p><fieldset class="ui-fieldset-reset" :disabled="busy"><div class="salon-settings-fields"><label class="salon-settings-wide">Название тарифа<input class="crm-input" v-model="draft.name" maxlength="100" required/></label><label>Цена за месяц, ₽<input class="crm-input" v-model.number="draft.monthlyPrice" type="number" min="0" max="1000000" step="0.01" required/></label><label>Цена за год, ₽<input class="crm-input" v-model.number="draft.annualPrice" type="number" min="0" max="10000000" step="0.01" required/></label></div><label class="salon-subscription-switch crm-toggle-row"><input class="crm-check" v-model="draft.freeAccess" type="checkbox"/>Бесплатный режим тарифа</label></fieldset><footer class="salon-settings-save"><span role="status">{{notice||(dirty?'Есть несохранённые изменения':'Все изменения сохранены')}}</span><button class="crm-button" type="submit" :disabled="busy||!dirty"><Save :size="18"/>Сохранить тариф</button></footer></form>
 </main>
</template>

...
<template>
  <main class="contact-inbox" :aria-busy="loading || saving">
    <header class="contact-inbox__head"><div><span class="contact-inbox__eyebrow">МАРКЕТИНГ / САЙТ</span><h1>Сообщения с сайта</h1><p>Обращения из формы на странице «Контакты» — в одном месте.</p></div><button type="button" class="contact-inbox__button" :disabled="loading" @click="load"><RefreshCw :size="18" /> Обновить</button></header>
    <p v-if="error" class="contact-inbox__error" role="alert">{{ error }}</p>
    <p v-if="notice" class="contact-inbox__notice" role="status">{{ notice }}</p>

    <section class="contact-inbox__settings" aria-labelledby="contact-inbox-settings-title">
      ...
      <form class="contact-inbox__settings-form" @submit.prevent="saveSettings"><label for="contact-recipient">Почта получателя</label><div><input id="contact-recipient" v-model="recipient" type="email" autocomplete="email" placeholder="manager@example.ru" :disabled="!canEditSettings || saving" /><button v-if="canEditSettings" type="submit" class="contact-inbox__button contact-inbox__button--dark" :disabled="saving || !settings || recipient.trim() === settings.recipientEmail"><Save :size="17" /> Сохранить</button></div><small v-if="settings">{{ !settings.recipientEmail ? 'Адрес не задан: сообщения остаются только в этом разделе.' : settings.deliveryEnabled ? 'Почтовая доставка включена. Статус конкретных писем проверяйте в почтовой очереди.' : 'Почтовая доставка сейчас выключена в конфигурации сервера. Сообщения сохраняются и в админке, и в очереди писем.' }}</small></form>
    </section>

    <section class="contact-inbox__panel" aria-labelledby="contact-inbox-list-title">
      <header class="contact-inbox__panel-head"><div><h2 id="contact-inbox-list-title">Входящие <span v-if="data">{{ data.total }}</span></h2><p v-if="data">Непрочитанных: {{ data.unread }}</p></div><label class="contact-inbox__filter"><input v-model="unreadOnly" type="checkbox" @change="page = 1" /> Только новые</label></header>

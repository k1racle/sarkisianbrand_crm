...
const { content, siteContent } = useStorefrontContent();
      ...
      <div v-if="open" class="sb-glass-layer" @click.self="emit('close')">
        <aside class="sb-side-drawer sb-menu-drawer" role="dialog" aria-modal="true" aria-label="Меню магазина">
          ...
          <div class="sb-menu-contact">
            <h3>Мы на связи</h3>
            <a :href="siteContentPhoneHref(siteContent.contacts.phone)">{{ siteContent.contacts.phone }}</a>
            <a :href="`mailto:${siteContent.contacts.email}`">{{ siteContent.contacts.email }}</a>

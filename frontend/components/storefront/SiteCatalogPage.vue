...
let syncingRoute = 0;
watch(query, value => { const { limit, ...params } = value; const next = Object.fromEntries(Object.entries(params).filter(([k,v]) => v !== undefined && !(k === 'sort' && v === 'new') && !(k === 'page' && v === 1))); syncingRoute++; router.replace({ path: model.categories.length === 1 ? storefrontCatalogLink(model.categories[0]) : '/catalog', query: model.categories.length === 1 ? { ...next, category: undefined } : next }).catch(() => undefined).finally(() => { syncingRoute--; }); });

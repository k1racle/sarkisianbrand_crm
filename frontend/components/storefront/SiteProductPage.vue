...
import { catalogProductCategory } from '~/shared/catalog-paths';
import { storefrontVariantPrice } from '~/shared/product-merchandising';

const props = defineProps<{ product: any }>();
const product = computed(() => props.product);
const productCategory = computed(() => catalogProductCategory(product.value));
...
useStorefrontSeo({
  ...
  image: () => image.value,
  canonical: () => storefrontProductLink(product.value),
});
</script>
  ...
  <SiteShell>
    <div v-if="product" class="sb-product-page">
      <div class="sb-breadcrumbs">
        <NuxtLink to="/">Главная</NuxtLink><span>/</span><NuxtLink to="/catalog">Каталог</NuxtLink><span>/</span><template v-if="productCategory"><NuxtLink :to="storefrontCatalogLink(productCategory)">{{productCategory.nameRu}}</NuxtLink><span>/</span></template><span>{{ product.nameRu }}</span>
      </div>

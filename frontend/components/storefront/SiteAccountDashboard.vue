...
const tabs = computed(() => [{ id: 'overview', label: 'Обзор', icon: UserRound }, { id: 'orders', label: 'Мои заказы', icon: Package }, { id: 'giftcards', label: 'Подарочные карты', icon: Gift }, ...(hasLoyalty.value ? [{ id: 'loyalty', label: 'Бонусы', icon: Award }] : []), { id: 'addresses', label: 'Адреса', icon: MapPin }, { id: 'profile', label: 'Мои данные', icon: Settings }, { id: 'notifications', label: 'Уведомления', icon: Bell }]);
...
<template>
  <div class="sb-account-page sa-ui">
    ...
    <template v-else-if="dashboard && profile">
      ...
      <div class="sb-account-layout"><nav class="sb-account-nav" aria-label="Разделы личного кабинета"><button v-for="tab in tabs" :key="tab.id" :class="{ active: active === tab.id }" :aria-current="active === tab.id ? 'page' : undefined" @click="active = tab.id"><component :is="tab.icon" :size="20" />{{ tab.label }}</button><button @click="signOut"><LogOut :size="20" />Выйти</button></nav><section class="sb-account-content" :aria-label="tabs.find(tab => tab.id === active)?.label">

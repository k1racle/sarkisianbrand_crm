...
<template>
  <section class="sb-gift-admin" aria-label="Управление подарочными картами">
    <header class="sb-gift-panel sb-gift-hero"><div><p class="sb-gift-eyebrow">ПОДАРКИ И БАЛАНСЫ</p><h2>Подарочные карты</h2><p>Настройки продажи и выданные карты. Финансовая история сохраняется — удаление карт недоступно.</p></div><button class="sb-gift-button sb-gift-button--white" :disabled="busy || loading" @click="load(true)"><RefreshCw :size="18" /> Обновить</button></header>
    <nav class="sb-gift-tabs" aria-label="Разделы подарочных карт"><button :class="{ active: tab === 'product' }" :aria-current="tab === 'product' ? 'page' : undefined" :disabled="busy" @click="selectTab('product')">Товар и номиналы</button><button v-if="canManageCards" :class="{ active: tab === 'cards' }" :aria-current="tab === 'cards' ? 'page' : undefined" :disabled="busy" @click="selectTab('cards')">Выданные карты</button></nav>
    ...
    <form v-if="tab === 'product' && product && !loading" class="sb-gift-panel sb-gift-form" novalidate @submit.prevent="saveProduct">
      ...
      <p class="sb-gift-note">Просмотр настроек ничего не создаёт. При сохранении создаётся или обновляется отдельный товар с вариантами по номиналам.</p>
      <fieldset :disabled="busy" class="sb-gift-fields"><label class="sb-gift-field sb-gift-wide"><span>Название товара</span><input v-model="product.nameRu" maxlength="160" required /></label><label class="sb-gift-field sb-gift-wide"><span>Описание для покупателей</span><textarea v-model="product.descriptionRu" maxlength="20000" rows="4" /></label><div class="sb-gift-field sb-gift-wide"><AdminMediaPicker v-model="product.imageUrl" :disabled="busy" label="Изображение карты" /></div>
      <section class="sb-gift-wide sb-gift-denominations"><header class="sb-gift-form-head"><h3>Номиналы, ₽</h3><button type="button" class="sb-gift-button sb-gift-button--white" :disabled="product.denominations.length >= 30" @click="addDenomination"><Plus :size="18" /> Добавить номинал</button></header><div v-for="(item, index) in product.denominations" :key="item.key" class="sb-gift-denomination"><label class="sb-gift-field"><span>Номинал {{ index + 1 }}, ₽</span><input v-model="item.value" type="number" inputmode="numeric" min="1" max="1000000" step="1" placeholder="1000" /></label><button type="button" class="sb-gift-icon-button" :aria-label="`Убрать номинал ${index + 1}`" @click="product.denominations.splice(index, 1)"><X :size="18" /></button></div><small>Целые рубли от 1 до 1 000 000, без повторов. Хотя бы один номинал. Изменения применятся только после сохранения.</small></section>
      <label class="sb-gift-field"><span>Срок действия после выдачи</span><select :value="validityPreset(product.validityDays)" @change="selectValidity(product, ($event.target as HTMLSelectElement).value)"><option value="90">90 дней</option><option value="180">180 дней</option><option value="365">1 год — 365 дней</option><option value="730">2 года — 730 дней</option><option value="custom">Другой срок</option></select></label><label class="sb-gift-field"><span>Срок действия, дней</span><input v-model="product.validityDays" inputmode="numeric" required /><small>От 1 до 3650 дней. Срок уже выданных карт здесь не меняется.</small></label><label class="sb-gift-checkbox sb-gift-wide"><input v-model="product.isActive" type="checkbox" /> Подарочная карта продаётся на сайте</label></fieldset>
      <footer class="sb-gift-actions"><button type="submit" class="sb-gift-button" :disabled="busy"><Save :size="18" /> {{ busy ? 'Сохраняем…' : 'Сохранить настройки' }}</button></footer>
    </form>
    <div v-if="tab === 'cards' && canManageCards" class="sb-gift-panel sb-gift-list"><header class="sb-gift-list-head"><label class="sb-gift-field"><span>Поиск на текущей странице</span><input v-model="search" type="search" maxlength="160" placeholder="Код или заметка" /></label><button class="sb-gift-button" :disabled="busy || loading" @click="openEditor()"><Plus :size="18" /> Выдать карту</button></header>
      <table v-if="visibleCards.length" class="sb-gift-table"><caption class="sb-gift-sr-only">Выданные карты и их балансы</caption><thead><tr><th>Карта</th><th>Номинал и баланс</th><th>Выдача и срок</th><th>Статус</th><th>Действия</th></tr></thead><tbody><tr v-for="card in visibleCards" :key="card.id"><td data-label="Карта"><div class="sb-gift-cell"><strong>{{ card.maskedCode }}</strong><span v-if="card.label">{{ card.label }}</span><small>Версия {{ card.revision }}</small></div></td><td data-label="Баланс"><div class="sb-gift-cell"><strong>{{ money(card.balance) }}</strong><small>Номинал: {{ money(card.faceValue) }}</small><small>Зарезервировано: {{ money(card.reserved) }}</small></div></td><td data-label="Срок"><div class="sb-gift-cell"><small>Выдана: {{ date(card.issuedAt) }}</small><small>Действует до: {{ date(card.expiresAt) }}</small></div></td><td data-label="Статус"><span class="sb-gift-badge">{{ status(card) }}</span></td><td data-label="Действия"><div class="sb-gift-row-actions"><button class="sb-gift-icon-button" :disabled="busy" :aria-label="`Редактировать карту ${card.maskedCode}`" title="Редактировать" @click="openEditor(card)"><Pencil :size="18" /></button><button class="sb-gift-icon-button" :disabled="busy" :aria-label="`Открыть код карты ${card.maskedCode}`" title="Открыть полный код" @click="openDialog('reveal', card)"><Eye :size="18" /></button><button class="sb-gift-button sb-gift-button--white sb-gift-history-button" :disabled="busy" :aria-label="`История карты ${card.maskedCode}`" @click="openDialog('history', card)"><History :size="18" /> История</button><button v-if="card.isActive" class="sb-gift-icon-button" :disabled="busy" :aria-label="`Выключить карту ${card.maskedCode}`" title="Выключить с сохранением истории" @click="openDialog('deactivate', card)"><Power :size="18" /></button></div></td></tr></tbody></table>
      ...
      <nav v-if="pageCount > 1" class="sb-gift-actions sb-gift-pagination" aria-label="Страницы выданных карт"><button class="sb-gift-button sb-gift-button--white" :disabled="busy || loading || page === 1" @click="changePage(page - 1)">Назад</button><span>Страница {{ page }} из {{ pageCount }}</span><button class="sb-gift-button sb-gift-button--white" :disabled="busy || loading || page >= pageCount" @click="changePage(page + 1)">Далее</button></nav>
    </div>
    <form v-if="tab === 'cards' && canManageCards && (issue || cardEditor)" ref="editorEl" class="sb-gift-panel sb-gift-form" novalidate @submit.prevent="saveCard"><header class="sb-gift-form-head"><div><h3>{{ issue ? 'Ручная выдача карты' : `Карта ${cardEditor?.maskedCode}` }}</h3><small>{{ editorDirty ? 'Есть несохранённые изменения' : 'Заполните параметры и сохраните' }}</small></div><button type="button" class="sb-gift-icon-button" :disabled="busy" aria-label="Закрыть редактор карты" @click="closeEditor"><X :size="20" /></button></header>
      <fieldset :disabled="busy" class="sb-gift-fields"><template v-if="issue"><p class="sb-gift-note sb-gift-wide">Ручная выдача создаёт финансовое обязательство компании. Укажите основание; это не тестовый предпросмотр и не продажа с оплатой.</p><label class="sb-gift-field"><span>Номинал, ₽</span><input v-model="issue.nominal" type="number" inputmode="numeric" min="1" max="1000000" step="1" placeholder="1000" required /></label><label class="sb-gift-field"><span>Срок действия, дней</span><input v-model="issue.validityDays" inputmode="numeric" required /></label><label class="sb-gift-field sb-gift-wide"><span>Код карты</span><div class="sb-gift-code-input"><input v-model="issue.code" maxlength="63" spellcheck="false" autocapitalize="characters" placeholder="Сгенерируется при выдаче" /><button type="button" class="sb-gift-button sb-gift-button--white" @click="generate"><Shuffle :size="18" /> Сгенерировать</button></div><small>Генерация кода сама по себе не выдаёт карту. Код скрывается после сохранения.</small></label><label class="sb-gift-field sb-gift-wide"><span>Заметка</span><input v-model="issue.label" maxlength="160" placeholder="Получатель или назначение" /></label><label class="sb-gift-field sb-gift-wide"><span>Причина ручной выдачи</span><textarea v-model="issue.reason" rows="3" maxlength="500" placeholder="Например, компенсация по согласованию руководителя" required /><small>Обязательное поле для финансовой истории.</small></label></template>
      <template v-else-if="cardEditor"><label class="sb-gift-field sb-gift-wide"><span>Заметка</span><input v-model="cardEditor.label" maxlength="160" /></label><label class="sb-gift-field"><span>Действует до</span><input v-model="cardEditor.expiresAt" type="datetime-local" required /></label><label class="sb-gift-checkbox"><input v-model="cardEditor.isActive" type="checkbox" /> Карта включена</label><p class="sb-gift-note sb-gift-wide">Номинал и баланс не редактируются вручную. Финансовая история не удаляется. Изменения проверяются по версии карты.</p></template></fieldset>
        ...
        <section ref="dialogEl" class="sb-gift-admin sb-gift-panel sb-gift-dialog" :class="{ 'sb-gift-dialog--history': dialog.kind === 'history' }" role="dialog" aria-modal="true" aria-labelledby="sb-gift-dialog-title" aria-describedby="sb-gift-dialog-description">
          ...
          <template v-if="dialog.kind === 'history'">
            ...
            <nav v-if="historyPageCount > 1" class="sb-gift-actions sb-gift-pagination" aria-label="Страницы истории карты">
              <button class="sb-gift-button sb-gift-button--white" :disabled="historyLoading || historyPage === 1" @click="loadHistory(historyPage - 1)">Назад</button>
              <span>Страница {{ historyPage }} из {{ historyPageCount }}</span>
              <button class="sb-gift-button sb-gift-button--white" :disabled="historyLoading || historyPage === historyPageCount" @click="loadHistory(historyPage + 1)">Далее</button>
            </nav>
          </template>
          <div v-if="secret && revealedForId === dialog.card.id" class="sb-gift-secret"><code tabindex="0">{{ secret }}</code><button class="sb-gift-button sb-gift-button--white" @click="copyCode"><Copy :size="18" /> Копировать код</button></div>
          <p v-if="error" class="sb-gift-notice" role="alert">{{ error }}</p>
          <footer class="sb-gift-actions">
            <button v-if="dialog.kind === 'history' && error" class="sb-gift-button sb-gift-button--white" :disabled="historyLoading" @click="loadHistory(historyPage)"><RefreshCw :size="18" /> Повторить</button>
            <button v-if="dialog.kind !== 'history' && !secret" class="sb-gift-button" :disabled="busy" @click="applyDialog">{{ busy ? 'Выполняем…' : dialog.kind === 'reveal' ? 'Показать полный код' : 'Выключить карту' }}</button>
            <button class="sb-gift-button sb-gift-button--white" :disabled="busy" @click="closeDialog()">{{ dialog.kind === 'history' ? 'Закрыть' : secret ? 'Скрыть и закрыть' : 'Отмена' }}</button>
          </footer>
        </section>
      </div>
    </Transition></Teleport>

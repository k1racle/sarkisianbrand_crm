...
import { homeSectionKeys as HOME_SECTION_KEYS, mergeSiteContent as resolveSiteContent, safeSiteContentUrl, type HomeSection as HomeSectionKey, type SiteContent, type SiteContentInput } from '../../shared/site-content';
...
const sectionNames: Record<HomeSectionKey, string> = { hero: 'Главный баннер', categories: 'Категории', bestsellers: 'Бестселлеры', club: 'SARKISIAN CLUB', manifesto: 'О бренде — главный тезис', story: 'История и портрет', benefits: 'Преимущества' };
...
function validate() {
  ...
  if (home.order.length !== HOME_SECTION_KEYS.length || new Set(home.order).size !== HOME_SECTION_KEYS.length || home.order.some(key => !HOME_SECTION_KEYS.includes(key))) throw new Error('Главная должна содержать все семь разделов без повторений.');

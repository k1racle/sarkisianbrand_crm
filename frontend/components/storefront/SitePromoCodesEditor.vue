...
<template>
  <section class="sb-promotions-admin" aria-label="Управление промокодами">
    ...
    <div class="sb-promo-panel sb-promo-list crm-surface">
      <div class="sb-promo-filters"><label class="sb-promo-search crm-input-group"><Search :size="18" /><input class="crm-input" v-model="search" :disabled="busy" maxlength="160" aria-label="Поиск промокода" type="search" placeholder="Поиск по коду или названию" /></label><label class="sb-promo-status-filter"><span>Показывать</span><select class="crm-input" v-model="statusFilter" :disabled="busy" aria-label="Статус промокодов"><option value="all">Все промокоды</option><option value="active">Активные</option><option value="inactive">Выключенные</option><option value="scheduled">Запланированные</option><option value="expired">Истёкшие</option></select></label></div>
      ...
      <table v-else-if="filtered.length" class="sb-promo-table"><caption class="sb-promo-sr-only">Промокоды, условия и статистика использования</caption><thead><tr><th>Промокод</th><th>Скидка и условия</th><th>Срок действия</th><th>Использование</th><th>Действия</th></tr></thead><tbody>
        <tr v-for="item in filtered" :key="item.code">
          <td data-label="Промокод"><div class="sb-promo-cell"><strong class="sb-promo-code">{{ item.code }}</strong><span>{{ item.title }}</span><span class="sb-promo-badge crm-badge">{{ promoStatus(item) }}</span></div></td>
    ...
    <form v-if="draft" ref="formEl" class="sb-promo-panel sb-promo-form crm-surface" novalidate @submit.prevent="save">
      ...
      <fieldset :disabled="busy" class="sb-promo-fields">
        ...
        <label class="sb-promo-field sb-promo-field--wide"><span>Код для покупателя</span><div class="sb-promo-code-input"><input class="crm-input" v-model="draft.code" :disabled="draft.existing" maxlength="40" placeholder="WELCOME10" autocapitalize="characters" spellcheck="false" @blur="draft.code = draft.code.trim().toUpperCase()" /><button v-if="!draft.existing" type="button" class="sb-promo-button sb-promo-button--white crm-button" @click="generate"><Shuffle :size="18" /> Сгенерировать</button></div><small>{{ draft.existing ? 'Код существующего промокода не меняется.' : '3–40 символов: латинские буквы, цифры, дефис или подчёркивание. Пустой код сгенерируется при сохранении.' }}</small></label>
        <label class="sb-promo-field"><span>Тип скидки</span><select class="crm-input" v-model="draft.discountType"><option value="PERCENT">Процент от суммы товаров</option><option value="FIXED">Фиксированная сумма, ₽</option></select></label>
        <label class="sb-promo-field"><span>{{ draft.discountType === 'PERCENT' ? 'Размер скидки, %' : 'Размер скидки, ₽' }}</span><input class="crm-input" v-model="draft.amount" inputmode="decimal" placeholder="10" required /><small>{{ draft.discountType === 'PERCENT' ? 'От 0,01 до 100%.' : 'Сумма в рублях, до двух знаков после запятой.' }}</small></label>
        <label class="sb-promo-field"><span>Минимальная сумма заказа, ₽</span><input class="crm-input" v-model="draft.minimumAmount" inputmode="decimal" placeholder="0" /><small>0 — без минимальной суммы.</small></label>
        <label v-if="draft.discountType === 'PERCENT'" class="sb-promo-field"><span>Максимальная скидка, ₽</span><input class="crm-input" v-model="draft.maximumDiscount" inputmode="decimal" placeholder="Без ограничения" /><small>Ограничивает сумму процентной скидки. Необязательное поле.</small></label>
        <label class="sb-promo-field"><span>Общий лимит использований</span><input class="crm-input" v-model="draft.usageLimit" inputmode="numeric" placeholder="Без ограничения" /><small>Пустое поле — без лимита. Бронь при оформлении тоже учитывается.</small></label>
    ...
    <Teleport to="body"><Transition name="sb-promo-dialog"><div v-if="action" class="sb-promo-dialog-layer admin-dialog-backdrop" @click.self="closeAction()" @keydown="dialogKeys"><section ref="dialogEl" class="sb-promotions-admin sb-promo-panel sb-promo-dialog admin-dialog admin-dialog--modal crm-surface" role="dialog" aria-modal="true" aria-labelledby="sb-promo-dialog-title" aria-describedby="sb-promo-dialog-description">
      ...
      <label v-if="action.kind === 'delete'" class="sb-promo-field"><span>Введите код {{ action.item.code }} для подтверждения</span><input class="crm-input" v-model="confirmation" :disabled="busy" autocomplete="off" spellcheck="false" :placeholder="action.item.code" @keydown.enter.prevent="applyAction" /><small>Код должен совпадать полностью, включая регистр.</small></label>
      <p v-if="error" class="sb-promo-notice" role="alert">{{ error }}</p>
      <footer class="sb-promo-actions"><button class="sb-promo-button crm-button crm-button--primary" :disabled="busy || (action.kind === 'delete' && confirmation !== action.item.code)" @click="applyAction">{{ busy ? 'Выполняем…' : action.kind === 'delete' ? 'Удалить промокод' : 'Выключить промокод' }}</button><button v-if="action.kind === 'delete' && action.item.isActive && uses(action.item)" class="sb-promo-button sb-promo-button--white crm-button" :disabled="busy" @click="action.kind = 'deactivate'; confirmation = ''; error = ''"><Power :size="18" /> Выключить вместо удаления</button><button class="sb-promo-button sb-promo-button--white crm-button" :disabled="busy" @click="closeAction()">Отмена</button></footer>

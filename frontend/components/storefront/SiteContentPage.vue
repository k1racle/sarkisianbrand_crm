...
import { siteContentPhoneHref } from '~/shared/site-content';
...
const { content, siteContent, loadStorefrontContent } = useStorefrontContent();
  ...
  <article class="sb-content-page" :class="{ 'is-legal': legal, 'is-about': page.slug === 'about', 'is-club': club }">
    ...
    <header class="sb-content-hero">
      <div><p class="sb-kicker">{{ page.eyebrow }}</p><h1>{{ page.title }}</h1><p class="sb-content-lead">{{ page.lead }}</p>
        ...
        <div v-if="club" class="sb-content-actions"><button class="sb-primary" @click="openAuth('register')">{{ siteContent.home.club.joinLabel }} <ArrowRight :size="18" /></button><NuxtLink class="sb-primary" to="/account">Мой кабинет <ArrowRight :size="18" /></NuxtLink></div>
      </div>

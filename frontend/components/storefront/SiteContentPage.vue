  ...
  <article v-else class="sb-content-page" :class="{ 'is-legal': legal, 'is-about': page.slug === 'about', 'is-club': club, 'is-contacts': page.slug === 'contacts', 'is-delivery': page.slug === 'delivery' }">
    ...
    <header class="sb-content-hero">
      ...
      <div class="sb-content-hero__copy"><p class="sb-kicker">{{ page.eyebrow }}</p><h1>{{ legal ? page.title : heading.main }} <em v-if="!legal && heading.accent">{{ heading.accent }}</em></h1><p class="sb-content-lead">{{ page.lead }}</p>
        ...
        <div v-if="page.slug === 'delivery'" class="sb-content-actions"><NuxtLink class="sb-primary" to="/catalog">Перейти в каталог <ArrowRight :size="18" /></NuxtLink></div>
        ...
        <section v-if="page.slug === 'contacts'" class="sb-contact-cards" aria-label="Способы связи">
          <a v-if="contactPhone" :href="siteContentPhoneHref(contactPhone)" class="sb-contact-card"><span class="sb-contact-card__icon"><Phone :size="20" /></span><span class="sb-contact-card__text"><small>ПОЗВОНИТЬ НАМ</small><strong>{{ contactPhone }}</strong></span><ArrowRight class="sb-contact-card__arrow" :size="19" /></a>
          <a v-if="contactEmail" :href="`mailto:${contactEmail}`" class="sb-contact-card"><span class="sb-contact-card__icon"><Mail :size="20" /></span><span class="sb-contact-card__text"><small>НАПИСАТЬ НА ПОЧТУ</small><strong>{{ contactEmail }}</strong></span><ArrowRight class="sb-contact-card__arrow" :size="19" /></a>
        </section>
      </div>
      ...
      <SiteContactForm v-if="page.slug === 'contacts'" />
      <SitePageArt v-if="page.slug === 'delivery'" :kind="page.slug" :phone="contactPhone" :email="contactEmail" />
    ...
    <div class="sb-content-layout">
      <aside v-if="page.blocks?.length && !club && page.slug !== 'about' && page.slug !== 'contacts' && page.slug !== 'delivery'" class="sb-content-toc"><span>На этой странице</span><nav aria-label="Содержание страницы"><a v-for="block in page.blocks" :key="block.id" :href="`#${block.id}`">{{ block.title }}</a></nav></aside>

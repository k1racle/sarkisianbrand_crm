...
<template>
  <section class="admin-media-library aml-browser" aria-label="Медиабиблиотека">
    ...
    <div class="aml-dropzone" :class="{ 'aml-dropzone--active': dragDepth > 0, 'aml-dropzone--busy': busy }" @dragenter="dragEnter" @dragover="dragOver" @dragleave.prevent="dragDepth = Math.max(0, dragDepth - 1)" @drop="dropFile">
      ...
      <button type="button" class="aml-button aml-button--white crm-button" :disabled="busy" @click="chooseFile">Выбрать файл</button>
    </div>
    <p v-if="status" class="aml-message" role="status">{{ status }}</p>
          <div class="aml-toolbar"><label class="aml-search"><span>Поиск по названию</span><input class="crm-input" v-model="q" type="search" maxlength="120" :disabled="busy" placeholder="Название файла" @keydown.enter.prevent="load(1)" /></label><button type="button" class="aml-button aml-button--white crm-button" :disabled="busy || loading" @click="load(page)">Обновить список</button><span class="aml-total" role="status">{{ loaded ? `Файлов: ${total}` : 'Загрузка списка…' }}</span></div>
          <p v-if="error" class="aml-message" role="alert">{{ error }} <button type="button" class="aml-button aml-button--white crm-button" :disabled="busy" @click="load(page)">Повторить загрузку списка</button></p>
          ...
          <div v-else-if="loaded && assets.length" class="aml-grid" role="group" aria-label="Изображения медиабиблиотеки">
            <button v-for="asset in assets" :key="asset.id" type="button" class="aml-asset crm-button crm-card-action crm-item-card" :class="{ 'is-selected': selectedId === asset.id }" :disabled="!supported(asset) || busy" :aria-pressed="selectedId === asset.id" :aria-label="`Выбрать ${asset.originalName}`" @click="selectedId = asset.id">
          ...
          <nav v-if="pages > 1" class="aml-pagination" aria-label="Страницы медиабиблиотеки"><button type="button" class="aml-button aml-button--white crm-button" :disabled="loading || busy || page === 1" @click="load(page - 1)">Назад</button><span>Страница {{ page }} из {{ pages }}</span><button type="button" class="aml-button aml-button--white crm-button" :disabled="loading || busy || page === pages" @click="load(page + 1)">Далее</button></nav>
          <footer class="aml-footer"><span>{{ selected?.originalName || 'Изображение не выбрано' }}</span><div class="aml-actions"><button v-if="showCancel" type="button" class="aml-button aml-button--white crm-button" @click="emit('cancel')">Отмена</button><button type="button" class="aml-button crm-button" :disabled="!selected || !supported(selected) || loading || busy" @click="chooseSelected">Использовать изображение</button></div></footer>

...
<template>
  <section class="studio-orders" aria-label="Заказы интернет-магазина" :aria-busy="busy">
    <div class="studio-order-tools">
      <label class="studio-order-search crm-input-group"><Search :size="18" aria-hidden="true" /><span class="wn-sr-only">Поиск заказов</span><input class="crm-input" id="studio-orders-query" :value="search" type="search" aria-label="Поиск заказов" placeholder="Номер заказа или email клиента" @input="emit('update:search', ($event.target as HTMLInputElement).value)" /></label>
      <label class="studio-order-filter"><span class="wn-sr-only">Статус заказа</span><select class="crm-input" :value="status" aria-label="Статус заказа" @change="emit('update:status', ($event.target as HTMLSelectElement).value)"><option v-for="view in statuses" :key="view.id" :value="view.id">{{ view.label }}</option></select></label>
      <button v-if="search || status" type="button" class="studio-order-reset crm-button" @click="emit('update:search', ''); emit('update:status', '')"><X :size="16" /> Сбросить</button>
    ...
    <div class="studio-orders-scroll"><table class="studio-orders-table"><thead><tr><th scope="col">Заказ</th><th scope="col">Покупатель</th><th scope="col">Доставка</th><th scope="col" class="studio-order-amount">Сумма</th><th scope="col">Заказ и оплата</th><th scope="col">Передача в 1С</th><th scope="col"><span class="wn-sr-only">Открыть заказ</span></th></tr></thead><tbody><tr v-for="order in items" :key="order.id" @click="emit('open', order)" @contextmenu.prevent="emit('context', $event, order)">
      <td><button type="button" class="studio-order-open crm-button" @click.stop="emit('open', order)">{{ order.orderNumber }}</button><small>{{ date(order.createdAt) }}</small></td>

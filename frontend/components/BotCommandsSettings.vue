...
<template>
  <section data-v-ui-4d3f24510037 class="command-settings">
    <div data-v-ui-4d3f24510037 class="command-kpis">
      <article data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Команд</span><strong data-v-ui-4d3f24510037>{{ stats.total }}</strong><small data-v-ui-4d3f24510037>в общем реестре</small></article>
      <article data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Активно</span><strong data-v-ui-4d3f24510037>{{ stats.enabled }}</strong><small data-v-ui-4d3f24510037>доступны пользователям</small></article>
      <article data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Без авторизации</span><strong data-v-ui-4d3f24510037>{{ stats.public }}</strong><small data-v-ui-4d3f24510037>публичные сценарии</small></article>
      <article data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>С проверкой профиля</span><strong data-v-ui-4d3f24510037>{{ stats.protected }}</strong><small data-v-ui-4d3f24510037>привязаны к экосистеме</small></article>
    </div>
    ...
    <article data-v-ui-4d3f24510037 class="commands-panel">
      <header data-v-ui-4d3f24510037 class="commands-head">
        ...
        <div data-v-ui-4d3f24510037 class="head-actions"><label data-v-ui-4d3f24510037><Search data-v-ui-4d3f24510037 :size="15" /><input data-v-ui-4d3f24510037 id="bot-command-query" v-model="search" aria-label="Поиск команд ботов" placeholder="Команда или название" /></label><button data-v-ui-4d3f24510037 aria-label="Обновить команды" @click="load"><RefreshCw data-v-ui-4d3f24510037 :size="15" :class="{ spin: loading }" /></button><button data-v-ui-4d3f24510037 class="primary" @click="edit()"><Plus data-v-ui-4d3f24510037 :size="15" /> Новая команда</button></div>
      </header>
      <div data-v-ui-4d3f24510037 class="filters"><span data-v-ui-4d3f24510037>Аудитория</span><button data-v-ui-4d3f24510037 v-for="item in [{id:'ALL',label:'Все'},...audiences]" :key="item.id" :class="{ active: audienceFilter === item.id }" @click="audienceFilter = item.id">{{ item.label }}</button><i data-v-ui-4d3f24510037></i><span data-v-ui-4d3f24510037>Канал</span><button data-v-ui-4d3f24510037 v-for="item in [{id:'ALL',label:'Все'},...channels]" :key="item.id" :class="{ active: channelFilter === item.id }" @click="channelFilter = item.id">{{ item.label }}</button></div>
      <div data-v-ui-4d3f24510037 class="command-table">
        <div data-v-ui-4d3f24510037 class="command-row head"><span data-v-ui-4d3f24510037>Команда</span><span data-v-ui-4d3f24510037>Аудитории</span><span data-v-ui-4d3f24510037>Каналы</span><span data-v-ui-4d3f24510037>Доступ</span><span data-v-ui-4d3f24510037>Состояние</span><span data-v-ui-4d3f24510037></span></div>
        <div data-v-ui-4d3f24510037 v-for="item in filtered" :key="item.id" class="command-row" @dblclick="edit(item)" @contextmenu.prevent="commandMenu($event, item)">
          ...
          <div data-v-ui-4d3f24510037 class="row-actions"><button data-v-ui-4d3f24510037 title="Редактировать" @click="edit(item)"><Pencil data-v-ui-4d3f24510037 :size="14" /></button><button data-v-ui-4d3f24510037 title="Удалить" @click="remove(item)"><Trash2 data-v-ui-4d3f24510037 :size="14" /></button></div>
        </div>
    ...
    <aside data-v-ui-4d3f24510037 v-if="selected" class="drawer-backdrop admin-dialog-backdrop" @click.self="close">
      <form data-v-ui-4d3f24510037 class="command-drawer admin-dialog admin-dialog--drawer" @submit.prevent="save">
        <header data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037 class="bot-icon large"><Bot data-v-ui-4d3f24510037 :size="20" /></span><div data-v-ui-4d3f24510037><p data-v-ui-4d3f24510037 class="kicker">{{ selected.id ? 'РЕДАКТИРОВАНИЕ СЦЕНАРИЯ' : 'НОВЫЙ СЦЕНАРИЙ' }}</p><h2 data-v-ui-4d3f24510037>{{ selected.id ? draft.title : 'Создать команду' }}</h2></div><button data-v-ui-4d3f24510037 type="button" class="close" @click="close"><X data-v-ui-4d3f24510037 :size="18" /></button></header>
        <div data-v-ui-4d3f24510037 class="drawer-scroll admin-dialog-body">
          <div data-v-ui-4d3f24510037 class="two-fields"><label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Команда</span><input data-v-ui-4d3f24510037 v-model.trim="draft.command" placeholder="/orders" required pattern="/?[a-z0-9_]{2,32}" /></label><label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Порядок</span><input data-v-ui-4d3f24510037 v-model.number="draft.sortOrder" type="number" min="0" max="10000" /></label></div>
          <label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Название</span><input data-v-ui-4d3f24510037 v-model.trim="draft.title" required maxlength="80" placeholder="Мои заказы" /></label>
          <label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Описание</span><textarea data-v-ui-4d3f24510037 v-model.trim="draft.description" rows="3" maxlength="300" placeholder="Что увидит пользователь после вызова команды"></textarea></label>
          <label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Обработчик</span><input data-v-ui-4d3f24510037 v-model.trim="draft.handlerKey" required pattern="[A-Z0-9_]{2,64}" placeholder="CUSTOMER_ORDERS" @input="draft.handlerKey = draft.handlerKey.toUpperCase().replace(/[^A-Z0-9_]/g, '')" /><small data-v-ui-4d3f24510037>Системный код обработчика. Заглавные латинские буквы и подчёркивания.</small></label>
          <section data-v-ui-4d3f24510037 class="choice-section"><h3 data-v-ui-4d3f24510037>Кому доступна команда</h3><button data-v-ui-4d3f24510037 v-for="item in audiences" :key="item.id" type="button" :class="{ selected: draft.audiences.includes(item.id) }" @click="toggleArray('audiences', item.id)"><span data-v-ui-4d3f24510037><ShieldCheck data-v-ui-4d3f24510037 :size="16" />{{ item.label }}</span><CheckCircle2 data-v-ui-4d3f24510037 v-if="draft.audiences.includes(item.id)" :size="16" /></button></section>
          <section data-v-ui-4d3f24510037 class="choice-section"><h3 data-v-ui-4d3f24510037>В каких ботах работает</h3><button data-v-ui-4d3f24510037 v-for="item in channels" :key="item.id" type="button" :class="{ selected: draft.channels.includes(item.id) }" @click="toggleArray('channels', item.id)"><span data-v-ui-4d3f24510037><MessageCircleMore data-v-ui-4d3f24510037 :size="16" />{{ item.label }}</span><CheckCircle2 data-v-ui-4d3f24510037 v-if="draft.channels.includes(item.id)" :size="16" /></button></section>
          <label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037>Шаблон ответа</span><textarea data-v-ui-4d3f24510037 v-model="draft.responseTemplate" rows="5" maxlength="2000" placeholder="Необязательно. Можно использовать подготовленный системный обработчик."></textarea></label>
          <div data-v-ui-4d3f24510037 class="switches"><label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037><b data-v-ui-4d3f24510037>Требовать авторизацию</b><small data-v-ui-4d3f24510037>Проверять привязку мессенджера к профилю</small></span><input data-v-ui-4d3f24510037 v-model="draft.requiresAuth" type="checkbox" /></label><label data-v-ui-4d3f24510037><span data-v-ui-4d3f24510037><b data-v-ui-4d3f24510037>Команда включена</b><small data-v-ui-4d3f24510037>Показывать и обрабатывать команду</small></span><input data-v-ui-4d3f24510037 v-model="draft.isEnabled" type="checkbox" /></label></div>
          <p data-v-ui-4d3f24510037 v-if="error" class="form-error"><CircleAlert data-v-ui-4d3f24510037 :size="15" />{{ error }}</p>
        </div>
        <footer data-v-ui-4d3f24510037><button data-v-ui-4d3f24510037 type="button" @click="close">Отмена</button><button data-v-ui-4d3f24510037 class="primary" type="submit" :disabled="saving"><Save data-v-ui-4d3f24510037 :size="15" />{{ saving ? 'Сохраняем…' : 'Сохранить команду' }}</button></footer>
      </form>
    </aside>
